   ...
   <div class="d-flex justify-center pt-1">
    ...
    <div class="wrapper__form" v-if="activeTab == 'Register'">
        ...
        <bib-form-group class="pt-05" label="First Name ">
            <template v-slot:input>
                <bib-input :value="userInfo.fname" @change="(event) => { userInfo.fname = event.target.value }" placeholder="Enter email..."></bib-input>
            </template>
        </bib-form-group>
        
        <bib-form-group class="pt-05" label="Last Name ">
            <template v-slot:input>
                <bib-input :value="userInfo.lname" @change="(event) => { userInfo.lname = event.target.value }" placeholder="Enter email..."></bib-input>
            </template>
        </bib-form-group>

        <bib-form-group class="pt-05" label="Telephone Number ">
            <template v-slot:input>
                <bib-input :value="userInfo.phone" @change="(event) => { userInfo.phone = event.target.value }" placeholder="Enter email..."></bib-input>
            </template>
        </bib-form-group>
        ...
        <bib-form-group required class="pt-05" label="Password ">
         ...
         <bib-form-group required class="pt-05" label="Confirm Password ">
            <template v-slot:input>
                <bib-input :value="userInfo.confirmPassword" @change="(event) => { userInfo.confirmPassword = event.target.value }" type="password" placeholder="*********"></bib-input>
            </template>
        </bib-form-group>
      
        <div class="wrapper__form__submit">
            <bib-button @click.native="register" label="Register" class="text-center" size="md" variant="success"></bib-button>
        </div>
        <div class="text-gray6 pt-1">
            <small>By selecting Register, you agree to our <a href="#">Terms of Use</a> and have read and acknowledge our <a href="#">Privacy Policy</a>.</small>
        </div>
    </div>
  </div>
</template>
