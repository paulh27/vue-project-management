    ...
    if (process.client) {
      ...
      if(p?.isDeleted != true) {
        ...
        if((this.userProj && JSON.parse(localStorage.getItem('user')).subr == 'USER') || JSON.parse(localStorage.getItem('user')).subr == 'ADMIN'){
            ...
            this.$store.dispatch("project/fetchTeamMember", { projectId: this.$route.params.id, userId: this.project.userId ? this.project.userId : null }).then(t => console.log(t.length, "members found")).catch(e => console.warn(e))
        } else {
