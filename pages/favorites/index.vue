<template>
    <div id="favorite_wrapper">
        <nav id="favorite-nav-wrapper" class="d-flex align-center gap-05 pt-05 p-5">
        ...
        <span class="font-lg font-w-700  mr-1" id="favorite-title-head">Favorites</span>
        </nav>
        ...
        <bib-table :fields="projectTableFields" class="border-gray4 bg-white" :sections="favoriteProjects" :hide-no-column="true" :collapseObj="{collapsed: false, label: 'Favorite Projects'}">
          ...
          <template #cell(status)="data">
            <div class="d-flex gap-05 align-center">
              <div class="shape-circle max-width-005 max-height-005 min-width-005 min-height-005" :class="'bg-' +             favoriteStatusVariable(data.value.projects.status ? data.value.projects.status.text : '')" :id="'projects-' + data.value.projects.statusId ? data.value.projects.statusId : ''">
              </div>
                <span :id="'projects-' + data.value.projects.statusId ? data.value.projects.statusId : '' + '-text'" class="text-dark text-truncate">{{ favoriteStatusLabel(data.value.projects.status ? data.value.projects.status : "") }}</span>
            </div>
          </template>
